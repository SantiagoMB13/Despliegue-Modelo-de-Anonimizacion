...
async function runNER(inputText) {
    ...
    try {
        ...
        for (const segment of segments) {
            ...
            replacedText += " " + await replaceEntitiesWithFakeData(segment, cleanedSegmentEntities);
        }
...
async function replaceEntitiesWithFakeData(text, entities) {
    let replacedText = text;

    for (const entity of entities) {
        let fakeData;
        if (entity.entity_group === 'PER') {
            const response = await axios.get('http://localhost:5000/fake_entity?type=name');
            fakeData = response.data.fake_data;
        } else if (entity.entity_group === 'LOC') {
            const response = await axios.get('http://localhost:5000/fake_entity?type=address');
            fakeData = response.data.fake_data;
        } else if (entity.entity_group === 'ORG' || entity.entity_group === 'MISC') {
            const response = await axios.get('http://localhost:5000/fake_entity?type=word');
            fakeData = response.data.fake_data;
        } else {
            const response = await axios.get('http://localhost:5000/fake_entity');
            fakeData = response.data.fake_data;
        }
        const regex = new RegExp(`\\b${entity.word}\\b`, 'g');
        replacedText = replacedText.replace(regex, fakeData);
    }
